import {
  REQUIRED_CATEGORIES,
  type Build, type Case, type Cooler, type Cpu, type Gpu, type Mobo,
  type Psu, type Ram, type Storage,
} from '../lib/types';
...
/** Estimated total system power draw under load, in watts (all components). */
export function totalSystemDraw(b: Build): number {
  return Object.values(b).reduce((sum, c) => sum + (c?.tdpWatts ?? 0), 0);
}

/**
 * Validates a build against real-world compatibility rules. Pure & deterministic
 * so the same logic can run server-side in the scoring Edge Function.
 */
export function validateBuild(b: Build): RuleResult[] {
  const results: RuleResult[] = [];
  const add = (ok: boolean, code: string, message: string) => results.push({ ok, code, message });
  ...
  if (cpu && mobo) add(cpu.socket === mobo.socket, 'SOCKET_MISMATCH',
  ...
  if (cpu && ram) add(cpu.memoryType === ram.memoryType, 'CPU_RAM_TYPE',
    `CPU's memory controller is ${cpu.memoryType} but the RAM is ${ram.memoryType}`);
  if (mobo && pcCase) add(pcCase.formFactorsSupported.includes(mobo.formFactor), 'MOBO_FIT',
    `${mobo.formFactor} motherboard doesn't fit in the ${pcCase.brand} ${pcCase.model}`);
  if (gpu && pcCase) add(gpu.dimensions.length <= pcCase.maxGpuLengthMm, 'GPU_LENGTH',
    `GPU is ${gpu.dimensions.length}mm long but the case only clears ${pcCase.maxGpuLengthMm}mm`);
  if (cooler && cpu) add(cooler.supportedSockets.includes(cpu.socket), 'COOLER_SOCKET',
  ...
  if (storage && mobo && storage.iface === 'NVMe') add(mobo.m2Slots >= 1, 'M2_SLOT',
    `${storage.model} needs an M.2 slot but the motherboard has none`);
  if (psu) {
